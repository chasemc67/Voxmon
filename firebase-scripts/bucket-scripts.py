...
cred = credentials.Certificate("./firebase-scripts/serviceAccount.json")
...
def update_voxmon_in_bucket():
    ...
    for item in items:
        #print(item.name)
        #print(item.metadata)
        if item.name in os.listdir(inputDir):
            print("replacing "+item.name)
            ...
            blob.upload_from_filename(inputDir+os.sep+name)
           # print("replacing: "+item.name)
           # item.delete()
        #   blob.upload_from_filename(inputDir+os.sep+item.name)
        #else:
